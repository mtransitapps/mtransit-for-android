...
interface VehicleLocationProviderPropertiesDao : BaseDao<VehicleLocationProviderProperties> {
    ...
    @Query("SELECT * FROM vehicle_location_provider_properties")
    ...
    @Query("SELECT * FROM vehicle_location_provider_properties WHERE authority = :authority")
    suspend fun getVehicleLocationProvider(authority: String): VehicleLocationProviderProperties

    @Query("SELECT * FROM vehicle_location_provider_properties WHERE target_authority = :targetAuthority")
