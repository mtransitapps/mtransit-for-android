...
public class UISchedule extends org.mtransit.android.commons.data.Schedule implements MTLog.Loggable {
	...
	@NonNull
	ArrayList<Timestamp> getNextTimestamps(
			long after,
	...
	private void addLastTimestamps(long after, ArrayList<Timestamp> timestamps) {
		...
		if (lastTimestamp != null && !timestamps.contains(lastTimestamp)) {
			if (!lastTimestamp.isNoPickup()
					|| lastTimestamp.getDepartureT() > after - TimeUnit.MINUTES.toMillis(30L)) {
	...
	private @NonNull ArrayList<DetailsNextDepartures> generateScheduleListTimes(
		...
		for (Timestamp t : timestamps) {
			...
			if (lastTimestamp > 0L) {
				...
				if (UITimeUtils.isSameDay(tTZ, after, departureT)) {
	...
	@NonNull
	public static SpannableStringBuilder decorateRealTime(
			@NonNull Context context,
			@NonNull Timestamp t,
	...
	@NonNull
	public static SpannableStringBuilder decorateOldSchedule(
			@NonNull Timestamp t,
			@NonNull SpannableStringBuilder timeSSB
	) {
		if (t.isOldSchedule()) {
		...
		if (!nextTimestampsT.isEmpty()) {
			...
			if (oldestUsefulTimestamp != null) {
				...
				while (it.hasNext()) {
					...
					if (timestamp.getDepartureT() < oldestUsefulTimestamp) {
						it.remove();
					}
				}
			}
