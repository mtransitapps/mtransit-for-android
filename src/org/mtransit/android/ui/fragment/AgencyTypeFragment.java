...
public class AgencyTypeFragment extends ABFragment implements ViewPager.OnPageChangeListener, MTActivityWithLocation.UserLocationListener {
	...
	private AgencyPagerAdapter agencyPagerAdapter;
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		showLoading();
		if (this.agencyPagerAdapter == null) {
			initTabsAndViewPager();
			if (this.agencyPagerAdapter != null && this.agencyPagerAdapter.getCount() > 0) {
	...
	private void initTabsAndViewPager() {
		...
		this.agencyPagerAdapter = new AgencyPagerAdapter(this, availableAgencies);
		final ViewPager viewPager = (ViewPager) getView().findViewById(R.id.viewpager);
		viewPager.setAdapter(this.agencyPagerAdapter);
	...
	public void onUserLocationChanged(Location newLocation) {
		if (newLocation != null) {
			...
			if (this.agencyPagerAdapter != null) {
				this.agencyPagerAdapter.setUserLocation(newLocation);
			}
		}
	}
	...
	public void onPageSelected(int position) {
		...
		if (this.agencyPagerAdapter != null) {
			PreferenceUtils.savePrefLcl(getActivity(), PreferenceUtils.getPREFS_LCL_AGENCY_TYPE_TAB_AGENCY(this.type.getId()), this.agencyPagerAdapter
					.getAgency(position).getAuthority(), false);
		}
		final List<Fragment> fragments = getChildFragmentManager().getFragments();
		if (fragments != null) {
			for (Fragment fragment : fragments) {
		...
		if (this.agencyPagerAdapter != null) {
			this.agencyPagerAdapter.setLastVisisbleFragmentPosition(this.lastPageSelected);
		}
	}
	...
	private static class AgencyPagerAdapter extends FragmentStatePagerAdapter {
		...
		public Fragment getItem(int position) {
			...
			if (agency.isRTS()) {
				return RTSAgencyRoutesFragment.newInstance(position, this.lastVisisbleFragmentPosition, agency, this.userLocation);
			}
